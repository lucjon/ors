...
Each of these problems is readily generalisable to finitely presented semigroups. Axel Thue discussed the word problem for semigroups as early as 1914; interestingly, by considering rewriting systems as we will in \cref{sec:rewriting-systems} rather than semigroups as algebraic structures\footnote{In \cite{Thue1914}; see the translation and commentary in \cite{Power2013}.}.
...
Two strings are related under the rewriting relation if the right hand side is the result of rewriting the left hand side by making exactly one replacement according to a rule in the corresponding rewriting system. Often it is useful to consider strings arising after an arbitrary number of replacements:
...
A corollary of this is that the rewriting algorithm must terminate for every string, as mentioned at the start of the section.

One way of showing a rewriting system is noetherian is to define a well-founded linear order $<$ on the set of strings $A^*$, and show that whenever $(u, v)$ is a rule in $R$, $v < u$. One common order which can be applied to many practical rewriting systems is the short-lexicographic order, which takes an order $\prec$ on the alphabet $A$ and extends it to a useful order on the whole set of strings $A^*$. Our examples thus far have mostly used the alphabet $\{a, b, c\}$, on which there is an `obvious' order to extend, $a \prec b \prec c$.
...
The crucial question we ask about this algorithm is then: does it halt on any input? Since $R$ is assumed to be noetherian, the string can only be reduced finitely many times. So the algorithm halts on any input as long as the number of rules in $R$ is finite (otherwise the loop in step \ref{nfal-for-every-rule} would never terminate). In fact, it is sufficient that there are only a finite number of rules which can apply to any particular $w'$, and that this set of rules can be found in finite time. For example, if the set of rules was stored in order of the length of the left hand side, and there are only finitely many rules of any given length, then we can simply check all rules until the length of one exceeds the length of $w'$.
...
We can now prove \cref{thm:ors-decidablewp}. We essentially use the argument in \cref{sec:normal-forms} using our confluent, noetherian rewriting system $R$. For \cref{alg:rewrite} to halt, we required that a finite number of rules applied to any given word $u \in A^*$, and that there was a procedure which could enumerate this set in a finite number of steps.

Start by simply enumerating all pairs $(a, b)$ of strings of length $|u|$ or less. It is decidable to compare $a$ and $b$ under the short-lex order, i.e. to determine if $b < a$. Furthermore, by Magnus' \cref{thm:orgp-decidablewp}, it is decidable whether $a =_G b$. If and only if both of these hold is $a \to b$ a rule in $R$. There are a finite number of these pairs, so we have our required procedure.

Given two words $u, v \in A^*$, we can thus obtain their normal forms $\bar u$ and $\bar v$ with respect to $R$ and, comparing these letter by letter, determine if $u =_R v$. Therefore it is decidable whether $u =_R v$.
