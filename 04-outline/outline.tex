...
Our motivation for doing so is the work of Ivanov, Margolis and Meakin in \cite{Ivanov2001}, who extended the algorithm described in \cref{sec:special-monoids} to show that a similar generating set to that we considered in \cref{sec:special-monoids} works for the group of units in a special inverse monoid. In that section, we solved the word problem of $M$ by reducing it to the corresponding problem for $G(M)$, so understanding this group in the context of inverse monoids could prove valuable in solving their word problem, and hence the word problem for every semigroup.
...
Later, in \cite{Adyan1987}, it was shown that the word problem is decidable for presentations of the form $\Sgp \langle A \mid a = w\rangle$, where $a \in A$ is a single letter and $w \in A^*$ is an arbitrary word.

Another set of results in a similar vain, due to a variety of authors, reduce the general word problem for semigroups to various special cases, in the hopes that these might be more tractable. One such reduction, proven by Adjan and Oganesyan in the 1980s \cite{Adyan1987}, states that the word problem is solved for all one-relation semigroups if it can be solved for those with presentations of the form $\Sgp \langle a, b \mid aua = bva \rangle$.

A more recent resulti from 2001, and the one that we will consider in this section, is that of Ivanov, Margolis and Meakin \cite{Ivanov2001}, which showed that the one-relation semigroup word problem is solved if the one-relation special inverse monoid word problem is solved; that is, if the word problem is decidable for inverse monoids with presentations of the form $\Inv \langle A \mid w = \epsilon\rangle$.
