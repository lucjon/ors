...
\begin{proof}
	...
	This is a homomorphism since $\phi$ is; injective since $u =_M v$ if and only if $\phi(u) =_{M'} \phi(v)$; and surjective since $\phi(E(M)^*) = B^*$. Therefore we have an isomorphism from $G(M)$ to $M'$ as required.
\end{proof}



\subsection{Right and left units}
